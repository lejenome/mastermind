...
enum {
  TAB_GAME,    ///< we will draw the game tab
...
void init_sdl() {
  ...
  if (TTF_Init() == -1) {
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "SDL_ttf cannont intialize",
                             TTF_GetError(), NULL);
    exit(EXIT_FAILURE);
  }
  ...
  if (font == NULL || icons == NULL) {
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Failed to load font",
                             TTF_GetError(), NULL);
    exit(EXIT_FAILURE);
  }
...
unsigned sdl_print(char *s, int x, int y, SDL_Color *color, int align) {
  ...
  SDL_Surface *surf = TTF_RenderUTF8_Solid(
      font, s,
      (color == NULL) ? default_color : *color); // create surface from font
  if (surf == NULL) {
...
unsigned sdl_print_icon(uint16_t c, int x, int y, SDL_Color *color) {
  ...
  SDL_Surface *surf;
  SDL_Color default_color = (SDL_Color)fg_color;
  surf = TTF_RenderGlyph_Blended(icons, c,
...
void setBg() {
  SDL_SetRenderDrawColor(rend, (SDL_Color)bg_color.r, (SDL_Color)bg_color.g,
                         (SDL_Color)bg_color.b, (SDL_Color)bg_color.a);
...
void initColors() {
  ...
  if (colors)
    free(colors);
  colors = (SDL_Color *)malloc(sizeof(SDL_Color) * session->config->colors);
  SDL_Color cl[] = {fg_red, fg_green, fg_yellow, fg_blue, fg_magenta, fg_cyan};
  for (i = 0; i < session->config->colors && i < LEN(cl); i++)
    colors[i] = (SDL_Color){cl[i].r, cl[i].g, cl[i].b, cl[i].a};
  for (; i < session->config->colors; i++)
    colors[i] =
        (SDL_Color){255 / (i + 1), (150 * 2) % 200, 100 / (i % 3 + 1), 255};
}
...
int drawTableBottom(SDL_Table *T) {
  ...
  for (i = 0; i <= T->rows; i++)
    SDL_RenderDrawLine(rend, T->x, T->y + (case_h * i), T->x + T->w,
                       T->y + (case_h * i));
  for (i = 0; i <= T->cols; i++)
    SDL_RenderDrawLine(rend, T->x + (w * i), T->y, T->x + (w * i), T->y + T->h);
  return 0;
}
...
void drawTableTop(SDL_Table *T) {
  ...
  for (i = 0; i <= T->cols + 1; i++)
    SDL_RenderDrawLine(rend, T->x + (case_w * i), T->y, T->x + (case_w * i),
                       T->y + T->h);
}
...
void drawCombination(uint8_t *G, unsigned p, unsigned drawState) {
  ...
  if (drawState) {
    ...
    sdl_print_center(s, state.x + state.w / 4, rect.y + rect.h / 3, &green);
    sprintf(s, "%d", session->panel[p].wrong_pos);
    sdl_print_center(s, state.x + (state.w / 4) * 3, rect.y + rect.h / 3,
                     &yellow);
  }
}
...
void redraw_settings() {
  ...
  for (conf = mm_confs; conf < mm_confs + MM_POS_LEN; conf++) {
    ...
    case MM_CONF_INT:
      ...
      sdl_print_icon(0xF0D7, SCREEN_WIDTH - case_w * 2.5, y, NULL);
      sdl_print_center(str, SCREEN_WIDTH - case_w * 2, y, NULL);
      sdl_print_icon(0xF0D8, SCREEN_WIDTH - case_w * 1.5, y, NULL);
      break;
    case MM_CONF_STR:
      sdl_print_center(conf->str.val, SCREEN_WIDTH - case_w * 2, y, NULL);
      break;
    }
    y += case_h;
  }
  button = (SDL_Table){.x = case_w * 0.5,
                       ...
                       .cols = 1,
                       .rows = 1};
  SDL_SetRenderDrawColor(rend, (SDL_Color)br_color.r, (SDL_Color)br_color.g,
                         (SDL_Color)br_color.b, (SDL_Color)br_color.a);
...
void redraw_game() {
  ...
  sdl_print_icon(0xF05A, control.x + button_w * 0.5, control.y + case_h / 2,
                 NULL);
  sdl_print_icon(0xF085, control.x + button_w * 1.5, control.y + case_h / 2,
                 NULL); // 0xF013
  sdl_print_icon(0xF097, control.x + button_w * 2.5, control.y + case_h / 2,
                 NULL);
  sdl_print_center(_("rest"), play.x + button_w, play.y + case_h / 2, NULL);
  sdl_print_center(_("play"), play.x + button_w * 3, play.y + case_h / 2, NULL);
...
int onMouseUp(SDL_MouseButtonEvent e) {
  ...
  if (curTab == TAB_SETTINGS) {
    if (e.x > case_w * 0.5 && e.x < button_w * 3 + case_w * 0.5 &&
      ...
      curTab = TAB_GAME;
      redraw();
    } else if (e.x > SCREEN_WIDTH - case_w * 3 && e.x < SCREEN_WIDTH - case_w &&
               e.y > case_h * 0.5 && e.y < case_h * MM_POS_LEN + case_h * 0.5) {
      i = (unsigned)(e.y - case_h * 0.5) / case_h;
      if (e.y < case_h * (i + 0.75) && e.y > case_h * (i + 1.25))
        return 1;
      switch (mm_confs[i].type) {
      case MM_CONF_BOOL:
        mm_confs[i].bool.val = mm_confs[i].bool.val == 0 ? 1 : 0;
        break;
      case MM_CONF_INT:
  ...
  if (e.x > panel.x && e.x < panel.x + panel.w &&
    ...
    if (e.x < control.x + button_w) {
      SDL_ShowSimpleMessageBox(
          ...
          _(PROGRAM_NAME ": Simple mastermind implemetation"), win);
      ...
      if (scores->len == 0)
        ...
        s = (char *)malloc(sizeof(char) * 42 * scores->len);
      for (i = 0; i < scores->len; i++)
        j += snprintf(s + j, 42, "%-2d) %-15ld %s\n", i, scores->T[i].score,
                      scores->T[i].account);
      SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, _("Scores"), s, win);
      if (scores->len)
        free(s);
    }
  }
...
int getGuess() {
  ...
  while (curPos < session->config->holes && SDL_PollEvent(&event) > 0 &&
    ...
    case SDL_KEYDOWN:
      ...
      if (curTab == TAB_GAME && event.key.keysym.sym >= SDLK_a &&
          event.key.keysym.sym < (session->config->colors + SDLK_a)) {
        curGuess[curPos++] = event.key.keysym.sym - SDLK_a;
        redraw();
...
void iter() {
  ...
  uint8_t *guess;
  if (curGuess == NULL) {
    curGuess = (uint8_t *)malloc(sizeof(uint8_t) * session->config->holes);
  ...
  play = getGuess();
  if (play == 0 && (session->state == MM_PLAYING || session->state == MM_NEW)) {
    guess = (uint8_t *)malloc(sizeof(uint8_t) * session->config->holes);
    for (curPos = 0; curPos < session->config->holes; curPos++)
      guess[curPos] = curGuess[curPos];
    curPos = 0;
    if (mm_play(session, guess)) { // entred a complete and valid guess
      SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_WARNING, _("Invalid Guess"),
