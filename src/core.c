...
void mm_config_load() {
  ...
  if ((f = fopen(mm_config_path, "r"))) {
    ...
    while (fscanf(f, "%40s %20s", n, v) != EOF) {
      ...
      // name as n (<name>)
      while (conf < mm_confs + LEN(mm_confs) && strcmp(n, conf->str.name) != 0)
        conf++;
      ...
      case MM_CONF_INT:        // config option is type integer
        ...
        if (t != NULL && errno != ERANGE && i >= conf->nbre.min &&
            i <= conf->nbre.max)
          conf->nbre.val = i;
        break;
      case MM_CONF_BOOL: // config option is type bool
        i = strtol(v, &t, 10);
        if (t != NULL && errno != ERANGE && (i == 1 || i == 0))
  ...
  if (mm_confs[MM_POS_REMISE].nbre.val &&
      mm_confs[MM_POS_COLORS].nbre.val < mm_confs[MM_POS_HOLES].nbre.val) {
...
unsigned mm_config_set(const char *name, const char *value) {
  ...
  while (conf < mm_confs + LEN(mm_confs) && strcmp(conf->str.name, name) != 0)
    conf++;
  if (conf == mm_confs + LEN(mm_confs))
    return 1;
  switch (conf->type) {
...
void mm_scores_save(mm_session *session) {
  ...
  for (i = 0; i < mm_scores.len; i++)
    fprintf(f, "%-15ld %s\n", mm_scores.T[i].score, mm_scores.T[i].account);
  fclose(f);
}
...
unsigned mm_play(mm_session *session, uint8_t *T) {
  ...
  uint8_t *freq = (uint8_t *)malloc(sizeof(uint8_t) * session->config->colors);
...
long unsigned mm_score(mm_session *session) {
  ...
  long unsigned score =
      session->config->holes * session->config->colors * MM_GUESSES_MAX * 2;
  ...
  for (i = 0; i < session->guessed; i++)
    score -= (session->config->holes * 2) -
             (session->panel[i].right_pos * 2 + session->panel[i].wrong_pos);
  // -%25 for remise mode
...
void mm_session_exit(mm_session *session) {
  if (session->state == MM_PLAYING && mm_confs[MM_POS_SAVE_EXIT].bool.val == 1)
    mm_session_save(session);
...
unsigned int mm_session_save(mm_session *session) {
  ...
  fwrite(session->secret->val, sizeof(uint8_t), session->config->holes, f);
  fwrite(session->secret->freq, sizeof(uint8_t), session->config->colors, f);
  for (i = 0; i < session->guessed; i++) {
...
mm_session *mm_session_restore() {
  ...
  if (!fread(session->secret->val, sizeof(uint8_t), session->config->holes, f))
    goto sec_val_err;
  session->secret->freq =
      (uint8_t *)malloc(sizeof(uint8_t) * session->config->colors);
  if (!fread(session->secret->freq, sizeof(uint8_t), session->config->colors,
             f))
    goto sec_freq_err;
  unsigned i = 0;
  session->panel =
