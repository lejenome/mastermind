...
unsigned int mm_session_save(mm_session *session) {
	...
	for(i=0; i < session->guessed; i++) {
		...
		fwrite(&session->panel[i].insecret, sizeof(unsigned char), 1, f);
	}
	fclose(f);
	free(_session);
	return 0;
...
mm_session *mm_session_restore() {
	...
	if(!fread(session->secret->val, sizeof(unsigned char),
				session->config->holes, f))
		goto sec_val_err;
	session->secret->freq = (unsigned char*)malloc(sizeof(unsigned char) *
			session->config->colors);
	...
	for(i=0; i < session->guessed; i++) {
		...
		if(!fread(session->panel[i].combination, sizeof(unsigned char),
					session->config->holes, f) ||
				!fread(&session->panel[i].inplace,
					sizeof(unsigned char),1, f) ||
				!fread(&session->panel[i].insecret,
					sizeof(unsigned char),1, f))
			goto guess_err;
	}
	fclose(f);
