  ...
  for (i = 0; i < session->config->guesses; i++) {
    ...
    if (i < session->guessed)
      printf(_("  | Y%2d | A%2d | N%2d |\n"),
             ...
             session->panel[i].right_pos, session->panel[i].wrong_pos,
             session->config->holes - session->panel[i].right_pos -
  ...
  if (argc == 0 ||
    ...
    for (i = 0; i < strlen(rl_line_buffer); i++)
      if (rl_line_buffer[i] >= '0' &&
        ...
        j++;
      else if (rl_line_buffer[i] != ' ' && rl_line_buffer[i] != '\t' &&
               rl_line_buffer[i] != ',')
        goto no_more;
...
int getCombination(uint8_t *T) {
  ...
  if (args[0][0] >= 'a' && args[0][0] <= 'z') {
    cmd = cmds;
    while (cmd < cmds + LEN(cmds) && strstr(cmd->n, args[0]) != cmd->n)
      cmd++;
...
int main(int argc, char *argv[]) {
  ...
  do {
    ...
    do {
      printPanel();
      T = (uint8_t *)malloc(sizeof(uint8_t) * session->config->holes);
